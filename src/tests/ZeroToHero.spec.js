...
test('mock immplementation of a function', () => {
...
test('expect a promise to resolve', async () =>{
    const user = {
        getFullName: jest.fn(() => Promise.resolve('Munya Denhere')),
    };
    await expect(user.getFullName('Munya Denhere')).resolves.toBe('Munya Denhere');
});
...
test('expect a promise to reject', async () =>{
    ...
    await expect(user.getFullName('Munya Denhere')).rejects.toThrow('Something went wrong');
});